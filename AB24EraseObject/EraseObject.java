...
public class EraseObject extends World<Tile> {
    ...
    /**
     * Constructs a default 20x20 grid containing black tiles at the locations given
     * in the file "digital.txt".
     */
    public EraseObject() {
    ...
    /**
     * Constructs an EraseObject World with the provided grid. Populates the world
     * with black tile objects at the locations given in the specified file (fName).
     * 
     * @param bg    bounded grid used by this World
     * @param fName name of file containing location information for tile placement
     *              in this World
     */
     ...
     * this grid.
     * 
     * The first entry in the provided text file is the number of pairs that follow
     * (i.e., 55). Each subsequent line contains a pair of integers, separated by a
     * blank space. Each pair is a row and column coordinate that specifies the
     * location of a black tile in the starting grid.
     * 
     * @param fName name of file containing tile locations
     * @throws FileNotFoundException
     */
    private void getData(String fName) throws FileNotFoundException {
    ...
    /**
     * Recursively changes the color of a series of adjacent tiles (erases) starting
     * at the given coordinates if this coordinate is part of the object. The method
     * should erase the entire object (remove black).
     *
    ...
    private void erase(Grid<Tile> gr, int row, int col) {
        ...
        if (gr.isValid(loc)) {
            if (gr.get(loc) != null && gr.get(loc).getColor() == IMGOBJ_COLOR) {
                gr.put(loc, new Tile(ERASE_COLOR));
                if (row == 0 || row == gr.getNumRows() || col == gr.getNumCols() - 1) {
