...
public class PollDisplayPanel extends JPanel {
    ...
    /**
     * Draws the pie chart. To avoid gaps in the picture, the following algorithm is
     * used: 1. set fromDegree to 0; 2. draw the red sector and increment fromDegree
     * by its size 3. draw the green sector and increment fromDegree by its size 4.
     * set the size of the blue sector to the remaining area, 360 - fromDegree, but
     * not less than 0: degrees = Math.max(360 - fromDegree, 0); (Occasionally, due
     * to rounding errors, fromDegree may become 361, for example, count1 = 5,
     * count2 = 11, count3 = 0.)
     * 
     * @param g Graphics context
    ...
    private void drawPieChart(Graphics g, int x, int y, int r) {
        ...
        if (total >= 0) {
            ...
            else { g.setColor(Color.LIGHT_GRAY); drawSector(g, x, y, r, fromDegree, 360);
           }
    ...
    /**
     * Returns the number of degrees in a pie slice that corresponds to count/total,
