...
public class KnightTourCritter extends KnightCritter {
     ...
     * with the lowest accessibility. It will return the location of first move with
     * the corresponding accessibility from the accessibility grid that is the
     * lowest. If there is a tie, it will return the location of smaller move value
     * in the list of locations. If all accessibility values are < 0, the method
     * returns null
     * 
    ...
    public Location getBestMove(ArrayList<Location> locs) {
        ...
        int moveAfter = accessGrid[bestMove.getRow()][bestMove.getCol()];
    ...
    /**
     * loc contains the coordinates of the latest square to be marked on the access
     * grid. The accessibility numbers of all valid moves reachable from loc are
     * decreased by 1. This algorithm will allow accessibility numbers to go
     * negative.
     * 
     * @param loc location of the latest square to be marked on the access grid
     */
     ...
     * (getLocation()) if <code>locs</code> has size 0. After the best move has been
     * chosen, the accessibility of the moves from the current location are updated
     * <br />
     * 
